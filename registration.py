...
On my/our honor, Lillian Pecoraro, this
...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_visit(start_vertex):
            """
            Helper method for DFS traversal and depth computation.
            """
            ...
            for next_index in self.get_adjacent_vertices(self.get_index(start_vertex.label)):
                ...
                # If the next vertex is unvisited, visit it
                if not next_vertex.visited:
                    dfs_visit(next_vertex)

                # Update depth of the current vertex
                start_vertex.depth = max(start_vertex.depth, 1 + next_vertex.depth)
    ...
    def get_registration_plan(self):
        ...
        while not courses.is_empty(): # Iterates until all courses have been scheduled
            ...
            for _ in range(4):
                if courses.is_empty():
                    break
                _, course_index = courses.delete()  # Remove the course from available courses
...
def main():
    ...
    vertices = int(data[position].strip())
    position += 1

    # read the vertices and add them into the graph
    for _ in range(vertices):
        label = data[position].strip()
        graph.add_vertex(label)
        position += 1

    # read the number of edges
    edges = int(data[position].strip())
    position += 1

    # read the edges and insert them into the graph
    for _ in range(edges):
        start_label, end_label = data[position].strip().split()
